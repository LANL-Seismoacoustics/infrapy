...
def run_loc(config_file, local_dets_in, back_az_width, range_max, resolution, pgm_file):
    ...
    if type(events[0]) is list:
        ...
        for j, det_list in enumerate(events):
            ...
            result = bisl.run(det_list, path_geo_model=pgm, resol=resolution, bm_width=back_az_width, rng_max=range_max, rad_min=100.0, rad_max=range_max/4.0)
        ...
        click.echo("")
        result = bisl.run(events, path_geo_model=pgm, resol=resolution, bm_width=back_az_width, rng_max=range_max, rad_min=100.0, rad_max=range_max/4.0)
