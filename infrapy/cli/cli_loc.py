...
def run_loc(config_file, local_detect_label, local_loc_label, back_az_width, range_max, resolution, src_est, pgm_file):
    ...
    if type(events[0]) is list:
        ...
        data_io.write_json(result, local_loc_label + ".loc.json")
...
@click.option("--yld-min", help="Minimum yield (default: 1 [tons eq. TNT])", default=1.0, type=float)
@click.option("--yld-max", help="Maximum yield (default: 1000 [tons eq. TNT])", default=1000.0, type=float)
@click.option("--ref-rng", help="Reference range for blastwave model (default 1 km)", default=1.0, type=float)
...
@click.option("--noise-option", help="Noise option ('pre', 'post', or 'beam')", default='post', type=str)
@click.option("--window-buffer", help="Window buffer scaling", default=0.2, type=float)
def run_yield(config_file, local_wvfrms, fdsn, db_config, local_detect_label, local_loc_label, local_yld_label, tlm_label, 
    ...
    yld_results = spye.run(det_list, smn_specs, src_loc, np.array([freq_min, freq_max]), models, 
                                                yld_rng=np.array([yld_min * 1e3, yld_max * 1e3]), ref_src_rng=ref_rng, resol=resolution)


    if ".yld.json" in local_yld_label:
        data_io.write_json(yld_results, local_yld_label)
    else:
        data_io.write_json(yld_results, local_yld_label + ".yld.json")


    print('\nResults:')
    print('\t' + "Maximum a Posteriori Yield:", yld_results['yld_vals'][np.argmax(yld_results['yld_pdf'])] / 1e3)
    print('\t' + "68% Confidence Bounds:", np.array(yld_results['conf_bnds'][0]) / 1e3)
    print('\t' + "95% Confidence Bounds:", np.array(yld_results['conf_bnds'][1]) / 1e3)
