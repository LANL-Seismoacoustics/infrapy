...
def plot_sd_single(trace, det_info, title=None, output_path=None, show_fig=False):

    t_shift = UTCDateTime(det_info['Time (UTC)']) - UTCDateTime(trace.stats.starttime)

    dt = trace.stats.delta
    nperseg = int((8.0 / det_info['Freq Range'][0]) / dt) 
    ...
    ax1.set_xlim([det_info['Start'] - 200.0, det_info['End'] + 200.0])
    ax1.set_yscale('log')
    ax1.set_ylabel("Frequency [Hz]")

    ax0 = fig.add_subplot(spec[2, :3], sharex=ax1, sharey=ax1)
    ax0.plot(Sxx_pnts[:, 0] - t_shift, Sxx_pnts[:, 1], '.', markersize=1.5, color='blue')
    ax0.set_yscale('log')
    ...
    ax3.semilogx(det_info['Sxx_det_max'][0], det_info['Sxx_det_max'][1], '-r', linewidth=1.0)
    ax3.axvspan(det_info['Freq Range'][0], det_info['Freq Range'][1], color='green', alpha=0.5, edgecolor=None)
    ax3.set_xlabel("Frequency [Hz]")
    ax3.set_ylabel("Spectral Amplitude [Pa^2/Hz]")
